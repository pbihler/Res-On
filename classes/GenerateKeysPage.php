 ...
 class GenerateKeysPage extends AdminPage {
     ...
     protected function renderNotes() {
         ...
         $this->writeJavascript('var current_key = 0; var max_key = 0; var crypt_module = "";' . 
                                ...
                                sprintf('      document.getElementById("key_output").innerHTML = "<form action=\"' . $this->KEY_PDF_PHP . '\" method=\"POST\" target=\"_blank\"><table id=\"key_output_table\"><tbody id=\"key_output_table_body\"><tr><th class=\"out\">%s</th><th class=\"out\">%s</th></tr></tbody></table><input type=\"checkbox\" name=\"minimal\" value=\"1\" id=\"minimal_check\"/><label for=\"minimal\"> %s</label><br /><input type=\"submit\" value=\"%s\" id=\"submit_data\" /><input type=\"hidden\" name=\"ext\" value=\".pdf\" /></form>&nbsp;";',Messages::getString('General.RKey'),Messages::getString('General.Password'),Messages::getString('GenerateKeysPage.GenerateMinimalPdf'),Messages::getString('GenerateKeysPage.GeneratePdf')) .
                                '      generate();' . 
                                '    } else { '. 
                                '      stop();' . 
                                '    }' . 
